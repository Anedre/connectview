...
// amplify/functions/admin-transfer-contact/handler.ts
var handler_exports = {};
...
async function audit(action, actor, target, result, errorMsg) {
  try {
    await dynamo.send(
  ...
  try {
    ...
    if (!flowId) {
      try {
        ...
        flowId = desc.Contact?.Arn ? process.env.DEFAULT_TRANSFER_FLOW_ID : void 0;
      } catch {
      }
    }
    if (!flowId) {
    ...
    const res = await connect.send(
      new import_client_connect.TransferContactCommand({
        InstanceId: INSTANCE_ID,
        ContactId: contactId,
        UserId: targetUserId,
