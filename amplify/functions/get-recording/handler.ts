...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    if (contact?.Recording?.Location) {
      const location = contact.Recording.Location;
      // Recording location format: connect/<instance>/<contactId>/<timestamp>.wav
      const bucketMatch = location.match(
        /^s3:\/\/([^/]+)\/(.+)$/
      );
      if (bucketMatch) {
    ...
    // Try to get transcription segments
    ...
    try {
      ...
      transcriptSegments =
        analysisResponse.Segments?.filter((s) => s.Transcript)?.map((s) => ({
          content: s.Transcript!.Content || "",
          participant: s.Transcript!.ParticipantRole || "UNKNOWN",
          beginOffsetMillis: s.Transcript!.BeginOffsetMillis || 0,
          endOffsetMillis: s.Transcript!.EndOffsetMillis || 0,
          sentiment: s.Transcript!.Sentiment || undefined,
        })) || [];
    } catch {
      console.log(`Transcription not available for contact ${contactId}`);
    }
    ...
    return {
      ...
      body: JSON.stringify({
        contactId,
        recordingUrl,
        duration: contact?.DisconnectTimestamp && contact?.InitiationTimestamp
          ? Math.round(
