...
export const handler = async (event: any) => {
  ...
  if (!tenantId) return { statusCode: 401, headers: HDRS, body: JSON.stringify({ error: "no autenticado" }) };
  ...
  try {
    ...
    if (method === "POST") {
      ...
      if (body.action === "runNow") {
        ...
        if (!exportId) return { statusCode: 400, headers: HDRS, body: JSON.stringify({ error: "exportId requerido" }) };
        const cur = await dynamo.send(new GetItemCommand({ TableName: TABLE, Key: { exportId: { S: exportId } } }));
        if (!cur.Item) return { statusCode: 404, headers: HDRS, body: JSON.stringify({ error: "no existe" }) };
        const row = unmarshall(cur.Item);
        if (!legacy && row.tenantId !== tenantId) return { statusCode: 403, headers: HDRS, body: JSON.stringify({ error: "ajeno" }) };
        await lambda.send(new InvokeCommand({ FunctionName: RUNNER, InvocationType: "Event", Payload: Buffer.from(JSON.stringify({ runNow: exportId })) }));
        return { statusCode: 202, headers: HDRS, body: JSON.stringify({ ok: true, running: exportId }) };
      }
      ...
      const recipients = Array.isArray(body.recipients)
        ? body.recipients.map((s: unknown) => String(s).trim()).filter((s: string) => /\S+@\S+\.\S+/.test(s))
        : [];
      if (recipients.length === 0) return { statusCode: 400, headers: HDRS, body: JSON.stringify({ error: "al menos un destinatario válido" }) };
      ...
      if (isNew) item.createdAt = now;
        ...
        if (cur.Item) {
          ...
          if (!legacy && prev.tenantId !== tenantId) return { statusCode: 403, headers: HDRS, body: JSON.stringify({ error: "ajeno" }) };
          item.createdAt = prev.createdAt || now;
          item.lastRunAt = prev.lastRunAt;
          item.lastStatus = prev.lastStatus;
        }
      }
      await dynamo.send(new PutItemCommand({ TableName: TABLE, Item: marshall(item, { removeUndefinedValues: true }) }));
    ...
    if (method === "DELETE") {
      ...
      await dynamo.send(new DeleteItemCommand({ TableName: TABLE, Key: { exportId: { S: exportId } } }));
      return { statusCode: 200, headers: HDRS, body: JSON.stringify({ ok: true }) };
    }

    return { statusCode: 405, headers: HDRS, body: JSON.stringify({ error: "método no permitido" }) };
  } catch (err) {
