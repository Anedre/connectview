...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function mapLead(fieldData: any[]): { phone?: string; name?: string; email?: string; attributes: Record<string, string> } {
  const attributes: Record<string, string> = {};
  let phone: string | undefined, name: string | undefined, email: string | undefined, firstName: string | undefined, lastName: string | undefined;
  for (const f of fieldData || []) {
    const k = String(f?.name || "").toLowerCase();
    const v = Array.isArray(f?.values) ? f.values[0] : f?.values;
    if (v == null || v === "") continue;
    if (k.includes("phone") || k === "telefono" || k === "celular" || k === "teléfono") phone = String(v);
    else if (k === "email" || k.includes("correo") || k.includes("e-mail")) email = String(v);
    else if (k === "full_name" || k === "name" || k === "nombre" || k === "nombre_completo") name = String(v);
...
async function handleLead(pageId: string, leadgenId: string, source: string): Promise<void> {
  ...
  let lead: any;
  try {
    lead = await graph(`${leadgenId}?fields=field_data,form_id,ad_id,campaign_name,platform,created_time`, token);
  } catch (e) {
  ...
  try {
    ...
    if (tc?.customerProfiles) setActiveProfiles(tc.customerProfiles, tc.customerProfilesDomain ?? "");
  ...
  try {
    ...
    console.log(`leadgen: tenant=${tenantId} phone=${phone} source=${src} lead=${result.leadId || "—"} action=${result.voxAction}`);
  } catch (e) {
...
async function backfill(tenantId: string, days: number): Promise<Record<string, number>> {
  ...
  do {
    const fres = await graph(`${pageId}/leadgen_forms?fields=id&limit=50${formsAfter ? `&after=${formsAfter}` : ""}`, pageToken);
    for (const f of fres.data || []) {
      out.forms++;
      let leadsAfter: string | undefined;
      do {
        const lres = await graph(`${f.id}/leads?fields=field_data,created_time,platform&limit=100${leadsAfter ? `&after=${leadsAfter}` : ""}`, pageToken);
        for (const lead of lres.data || []) {
          if (lead.created_time && new Date(lead.created_time).getTime() < sinceMs) { out.skipped++; continue; }
          out.leads++;
          const mapped = mapLead(lead.field_data);
          if (!mapped.phone && !mapped.email) { out.skipped++; continue; }
          const phone = normalizePhone(mapped.phone)?.e164 || mapped.phone || "";
          const src = lead.platform === "ig" || lead.platform === "instagram" ? "instagram" : "facebook";
          try {
            const r = await propagateLead(
              { phone, name: mapped.name, email: mapped.email, source: src, attributes: { ...mapped.attributes, meta_form_id: String(f.id) } },
              { origin: "vox" }
            );
        ...
        leadsAfter = lres.paging?.cursors?.after && lres.data?.length ? lres.paging.cursors.after : undefined;
      } while (leadsAfter);
    }
    formsAfter = fres.paging?.cursors?.after && fres.data?.length ? fres.paging.cursors.after : undefined;
  } while (formsAfter);
  return out;
}
...
export const handler: Handler = async (event: any) => {
  ...
  // Backfill (Fase C) — admin, gated por x-vox-internal.
  if (body.action === "backfill") {
    const hdrs = event?.headers || {};
    const internalOk = !!INTERNAL_SECRET && (hdrs["x-vox-internal"] || hdrs["X-Vox-Internal"]) === INTERNAL_SECRET;
    const JSONH = { "Content-Type": "application/json" };
    if (!internalOk) return { statusCode: 401, headers: JSONH, body: JSON.stringify({ error: "no autorizado" }) };
    const tenantId = String(body.tenantId || "");
    if (!tenantId) return { statusCode: 400, headers: JSONH, body: JSON.stringify({ error: "tenantId requerido" }) };
    ...
    try {
      ...
      return { statusCode: 500, headers: JSONH, body: JSON.stringify({ error: e instanceof Error ? e.message : "backfill failed" }) };
    }
  }
