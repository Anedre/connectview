...
export async function getConversation(dynamo: DynamoDBClient, conversationId: string): Promise<Conversation | null> {
  try {
    const r = await dynamo.send(new GetItemCommand({ TableName: CONV_TABLE, Key: { conversationId: { S: conversationId } } }));
...
function blank(channel: ConvChannel, senderId: string, now: string, tenantId?: string): Conversation {
  return {
...
async function put(dynamo: DynamoDBClient, conv: Conversation): Promise<void> {
  await dynamo.send(new PutItemCommand({ TableName: CONV_TABLE, Item: marshall(conv, { removeUndefinedValues: true }) }));
}

/** Mensaje entrante (del cliente) → upsert de la conversación + unread++. */
export async function appendInbound(
  dynamo: DynamoDBClient,
  m: { channel: ConvChannel; senderId: string; text: string; customerName?: string; ts?: string; tenantId?: string; attachment?: { type: string; url: string } }
): Promise<Conversation> {
  const now = m.ts || new Date().toISOString();
  const conv = (await getConversation(dynamo, convId(m.channel, m.senderId))) || blank(m.channel, m.senderId, now, m.tenantId);
  conv.messages = [...(conv.messages || []).slice(-199), { id: randomUUID(), direction: "in", text: m.text, ts: now, attachment: m.attachment }];
...
export async function appendOutbound(
  ...
  conv.messages = [...(conv.messages || []).slice(-199), { id: randomUUID(), direction: "out", text, ts: now, agent }];
  conv.unread = 0;
  conv.lastMessageAt = now;
...
/** Marca leída / asigna / cierra. */
export async function patchConversation(
  dynamo: DynamoDBClient,
  conversationId: string,
  patch: Partial<Pick<Conversation, "unread" | "status" | "assignedAgent">>
): Promise<Conversation | null> {
  const conv = await getConversation(dynamo, conversationId);
  if (!conv) return null;
...
export async function listConversations(dynamo: DynamoDBClient, opts: { limit?: number } = {}): Promise<Conversation[]> {
  const out: Conversation[] = [];
  let ESK: Record<string, unknown> | undefined;
  try {
    do {
      const r = await dynamo.send(new ScanCommand({ TableName: CONV_TABLE, ExclusiveStartKey: ESK as never }));
