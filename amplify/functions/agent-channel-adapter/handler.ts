...
interface BotMsg { kind: string; text?: string; buttons?: { id: string; label?: string; type?: string }[]; rows?: { id: string; title?: string; description?: string }[]; media?: { type: string; url: string; caption?: string } }
...
async function sendWhatsApp(to: string, metaMsg: Record<string, unknown>): Promise<Record<string, unknown>> {
  void to; // `to` ya viene embebido en metaMsg (buildMetaMessage).
  // Tenant real con número configurado → enviamos de verdad desde SU número,
  // con SUS creds assumed. El tenant ya opt-in al cargar su End User Messaging,
  // así que NO aplicamos el gate DRY_RUN (ese es solo para el número de Vox).
  if (waClient && waPhoneNumberId) {
    const out = await waClient.send(new SendWhatsAppMessageCommand({
      originationPhoneNumberId: waPhoneNumberId,
      metaApiVersion: "v20.0",
      message: new TextEncoder().encode(JSON.stringify(metaMsg)),
    }));
    return { messageId: (out as { messageId?: string }).messageId, tenantScoped: true };
  }
  ...
  const out = await social.send(new SendWhatsAppMessageCommand({
...
export const handler: Handler = async (event: any) => {
  ...
  if (p?.tenantId) {
    ...
    if (tc) {
      dynamo = tc.dynamo;
      // BYO WhatsApp: si el tenant cargó su número de End User Messaging,
      // las respuestas del agente salen desde SU número (no el de Vox).
  ...
  if (BOT_RUNTIME_URL) {
    try {
      const resp = await fetch(BOT_RUNTIME_URL, {
        ...
        body: JSON.stringify({ botId: agentBotId, state, input: message ? { text: message } : undefined, source: "whatsapp", toolEndpoints: TOOL_ENDPOINTS }),
      });
      d = await resp.json();
  ...
  // 3) persist new state
  try {
    await dynamo.send(new PutItemCommand({ TableName: CONV_TABLE, Item: marshall({ botId: sessKey, recType: "session", agentBotId, state: JSON.stringify(d.state || {}), updatedAt: new Date().toISOString() }, { removeUndefinedValues: true }) }));
  } catch { /* best-effort */ }
  ...
  if (to) {
    for (const m of botMsgs) {
      try { sent.push(await sendWhatsApp(to, buildMetaMessage(to, m))); }
      catch (e) { sent.push({ error: e instanceof Error ? e.message : String(e) }); }
    }
  }
  ...
  return {
    ...
    reply: botMsgs.map((m) => m.text || "").join("\n").slice(0, 900),
