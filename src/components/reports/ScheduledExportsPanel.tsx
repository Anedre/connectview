...
export function ScheduledExportsPanel() {
  ...
  const act = async (job: ExportJob, kind: "runNow" | "toggle" | "delete") => {
    ...
    try {
      if (kind === "delete") {
        const r = await authedFetch(`${url}?exportId=${encodeURIComponent(job.exportId)}`, {
          method: "DELETE",
        });
    ...
    return (
      ...
      >
        ...
        <div style={{ marginTop: 4, color: "var(--text-3)" }}>
          Ejecuta <code>node scripts/create-scheduled-exports.mjs</code> y pega la URL en{" "}
          <code>apiEndpoints.manageScheduledExports</code>.
        </div>
      </div>
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      >
        ...
        <div className="row" style={{ gap: 8 }}>
          ...
          {form.exportId && (
            <button className="btn btn--sm" onClick={() => setForm(blankForm)}>
              Cancelar
            </button>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="muted" style={{ padding: 16, textAlign: "center", fontSize: 12.5 }}>
          Todavía no hay exports programados.
        </div>
      ) : (
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div className="row" style={{ gap: 8, alignItems: "center" }}>
                  ...
                  <span className="chip" style={{ height: 17, fontSize: 10 }}>
                    {FREQS.find((f) => f.id === j.frequency)?.label} ·{" "}
                    {String(j.hourUtc).padStart(2, "0")}:00 UTC
                  </span>
                  {j.lastStatus && (
                    <span
                      className="chip"
                      style={{
                        ...
                        background:
                          j.lastStatus === "ok"
                            ...
                            : "var(--accent-red-soft)",
                        color: j.lastStatus === "ok" ? "var(--accent-green)" : "var(--accent-red)",
                      }}
                    >
                      {j.lastStatus === "ok" ? "última OK" : "última falló"}
                    </span>
                  )}
                </div>
                <div
                ...
                >
                  <Mail size={11} /> {j.recipients.join(", ")} · próximo {fmt(j.nextRunAt)}
                  {j.lastRunAt ? ` · última corrida ${fmt(j.lastRunAt)}` : ""}
                </div>
              </div>
              <button
                className={`chip ${j.enabled ? "chip--green" : ""}`}
                style={{ cursor: "pointer", border: "none" }}
                onClick={() => act(j, "toggle")}
                disabled={busy === j.exportId}
                title={j.enabled ? "Pausar" : "Activar"}
              >
                <span className="dot" /> {j.enabled ? "Activo" : "Pausado"}
              </button>
              <button
                className="btn btn--ghost btn--sm"
                onClick={() => act(j, "runNow")}
                disabled={busy === j.exportId}
