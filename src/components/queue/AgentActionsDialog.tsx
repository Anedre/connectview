...
export function AgentActionsDialog({ agent, statuses, open, onClose, onActionCompleted }: Props) {
  ...
  const handleStopCall = async () => {
    ...
    if (
      !(await confirm({
        title: `¿Terminar la llamada activa de ${agent.username}?`,
        destructive: true,
        confirmLabel: "Terminar llamada",
      }))
    )
      return;
    try {
  ...
  const quick = statuses
    .filter((s) => /available|disponible|break|descanso|lunch|almuerzo|offline/i.test(s.name))
    .slice(0, 4);
        ...
        <DialogHeader className="space-y-0 p-0">
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <DialogTitle style={{ fontSize: 18, fontWeight: 700, color: "var(--text-1)" }}>
                {agent.username}
              </DialogTitle>
              <div
                style={{
              ...
              >
                ...
                >
                  <span
                    style={{ width: 7, height: 7, borderRadius: "50%", background: stateColor }}
                  />
          ...
          {live && (
            ...
            >
              ...
              >
                ...
                >
                  {live.queueName ? `${live.queueName} · ` : ""}
                  {fmtDur(live.connectedToAgentTimestamp)}
                </span>
              </div>
              ...
              <p style={{ fontSize: 11, color: "var(--text-3)", marginTop: 8, marginBottom: 0 }}>
                Empieza escuchando en silencio; desde la barra inferior puedes pasar a intervenir o
                salir.
              </p>
            </div>
          )}
