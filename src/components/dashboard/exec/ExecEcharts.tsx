...
/* ---------- Embudo de leads (funnel trapezoidal) ---------- */
export function ExecFunnelEChart({
  data,
  height = 300,
}: {
  ...
  const option: EChartsOption = {
    ...
    tooltip: {
      ...
      formatter: (p: unknown) => {
        ...
        return `${it.name}<b style="margin-left:10px">${it.value}</b> · ${pct}% del total`;
      },
    },
    series: [
      {
        ...
        labelLine: { show: false },
        itemStyle: { borderColor: t.bg1, borderWidth: 2, borderRadius: 5 },
        emphasis: { label: { fontSize: 13 } },
        data: data.map((d) => ({
          name: d.label,
          value: d.value,
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          itemStyle: { color: vGrad(d.color, 1, 0.78) as any },
        })),
      },
    ],
  };
