...
export function WaTemplateConfigurator({
  ...
  const pickTemplate = (name: string) => {
    ...
    onChange({
      ...
      variables: new Array(t?.variableCount || 0).fill(""),
    });
  };
        ...
        >
          ...
          {templates.map((t) => (
            <option key={t.metaTemplateId || t.name} value={t.name}>
              ...
              {(t.variableCount || 0) > 0 ? ` · ${t.variableCount} var` : ""}
            </option>
          ))}
        </select>
      )}
        ...
        <>
          ...
          <div className="watc__preview">
            <div className="watc__bubble">
              ...
              <div className="watc__body">
                {previewBody || <span className="muted">(sin cuerpo)</span>}
              </div>
              ...
              <div className="watc__btns">
                {(tpl.buttons || []).map((b, i) => (
                  <div
                    key={i}
                    className="watc__btn"
                    title={b.url || b.phoneNumber || "Respuesta rápida"}
                  >
                    <span>
                      {btnIcon(b.type)} {b.text}
                    </span>
                    {(b.url || b.phoneNumber) && (
                      <span className="watc__btn-meta">{b.url || b.phoneNumber}</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          {varCount > 0 ? (
            <div className="watc__vars">
              <div className="watc__vars-h">
                {mode === "campaign"
                  ? columns.length === 0
                    ? "Variables → escribilas a mano, o usa el nombre del contacto"
                    : "Variables → se escriben a mano por defecto; cambia a “Columna” si alguna corresponde a tu lista"
                  : "Variables → variable del flujo o valor fijo"}
              </div>
              {Array.from({ length: varCount }).map((_, i) => (
                <div key={i} className="watc__var">
                          ...
                          <div className="watc__var-camp">
                            <div className="watc__toggle">
                              <button
                                ...
                                className={isLit ? "on" : ""}
                                onClick={() => {
                                  if (!isLit) setVar(i, "lit:");
                                }}
                                title="Escribir el valor a mano (igual para todos los contactos)"
                              >
                                Manual
                              </button>
                              <button
                                ...
                                className={!isLit ? "on" : ""}
                                onClick={() => {
                                  if (isLit) setVar(i, "__customerName__");
                                }}
                                title={
                                  noColumns
                                    ? "Usar el nombre del contacto (distinto por contacto)"
                                    : "Tomar el valor de una columna del lead (distinto por contacto)"
                                }
                              >
                                {noColumns ? "Nombre" : "Columna"}
                              </button>
                            </div>
                            {isLit ? (
                              <input
                                className="watc__var-input"
                                placeholder="Escribe el valor (igual para todos)…"
                                value={val.startsWith("lit:") ? val.slice(4) : ""}
                                onChange={(e) => setVar(i, "lit:" + e.target.value)}
                              />
                            ) : (
                              <select
                                className="watc__select watc__var-input"
                                value={val}
                          ...
                          <div className="watc__var-camp">
                            <div className="watc__toggle">
                              <button
                                ...
                                title="Tomar el valor de una variable capturada en el flujo (ej. la respuesta a una pregunta)"
                              >
                                Variable
                              </button>
                              <button
                                ...
                                title="Mismo valor fijo, escrito por ti"
                              >
                                Fijo
                              </button>
                            </div>
                            {isVar ? (
                              <div className="watc__fv">
                                <input
                                  className="watc__var-input"
                                  list={dlId}
                                  placeholder="nombre de variable (ej. carrera)"
                                  value={varName}
                                  onChange={(e) =>
                                    setVar(i, e.target.value ? `{{${e.target.value}}}` : "{{}}")
                                  }
                                />
                                {flowVars.length > 0 && (
                                  <datalist id={dlId}>
                                  ...
                                  <div className="watc__fv-chips">
                                    ...
                                    {flowVars.map((v) => (
                                      <button
                                        type="button"
                                        key={v}
                                        className={`watc__fv-chip${varName === v ? " on" : ""}`}
                                        onClick={() => setVar(i, `{{${v}}}`)}
                                        title={`Usar {{${v}}}`}
                                      >
                                        {v}
                  ...
                  {ctxFor(i) && (
                    <span className="watc__var-ctx" title={ctxFor(i)}>
                      {ctxFor(i)}
                    </span>
                  )}
                </div>
          ...
          {(tpl.buttons || []).length > 0 && (
            <div className="watc__note">
              Los botones son fijos (aprobados por Meta): respuesta rápida, enlace o llamada. No se
              editan aquí.
            </div>
          )}
        </>
