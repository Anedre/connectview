...
import { Phone, Clock, Disc3, PhoneIncoming, PhoneOutgoing, PhoneMissed, ChevronLeft, ChevronRight } from "lucide-react";
...
const realAgentOf = (r: CallRow) => (r.agentUsername && !looksUuid(r.agentUsername) ? r.agentUsername : "");
...
export function CallPlayerView({ phone, onActiveCall }: { phone: string | null; onActiveCall?: (c: ActiveCall | null) => void }) {
  ...
  useEffect(() => {
    setSelectedId(null); setSelectedDay(null); setMonth(null); setDir("all"); setAgent("all");
  }, [phone]);

  const callRows = useMemo(
    () => rows.filter((c) => { const ch = (c.channel || "").toUpperCase(); return ch === "VOICE" || ch === "TELEPHONY"; }),
    [rows]
  );
  const agents = useMemo(() => {
    const s = new Set<string>();
    callRows.forEach((c) => { if (c.agentUsername) s.add(c.agentUsername); });
    return [...s].sort();
  }, [callRows]);
  const filtered = useMemo(() => callRows.filter((r) => {
  ...
  const countsByDay = useMemo(() => {
    ...
    for (const r of filtered) { const d = ymdLocal(r.initiationTimestamp); if (d) m[d] = (m[d] || 0) + 1; }
    return m;
  }, [filtered]);
  ...
  const trends = useMemo(() => {
    ...
    const calls = new Array(WEEKS).fill(0), missed = new Array(WEEKS).fill(0);
    const durSum = new Array(WEEKS).fill(0), durCnt = new Array(WEEKS).fill(0);
    for (const r of filtered) {
      const t = Date.parse(r.initiationTimestamp); if (Number.isNaN(t)) continue;
      const wa = Math.floor((today.getTime() - t) / (7 * 86400000));
      if (wa < 0 || wa >= WEEKS) continue;
      const i = WEEKS - 1 - wa;
      calls[i]++;
      if ((r.duration || 0) === 0) missed[i]++; else { durSum[i] += r.duration; durCnt[i]++; }
    }
    return {
      calls, missed,
  ...
  const dayRows = useMemo(() => {
    ...
    return filtered
      ...
      .sort((a, b) => (Date.parse(a.initiationTimestamp) || 0) - (Date.parse(b.initiationTimestamp) || 0));
  ...
  const pickDay = (ymd: string) => {
    ...
    const first = filtered
      ...
      .sort((a, b) => (Date.parse(a.initiationTimestamp) || 0) - (Date.parse(b.initiationTimestamp) || 0))[0];
  ...
  const selected = filtered.find((c) => c.contactId === selectedId) || callRows.find((c) => c.contactId === selectedId) || null;

  return (
    <div className="cpv2">
      <div className="cpv2__stats">
        <div className="cpv2__stat">
          <div className="cpv2__stat-l">Llamadas</div><div className="cpv2__stat-v">{total}</div>
          <div className="cpv2__stat-spark"><Sparkline data={trends.calls} color="var(--accent-cyan)" /></div>
        </div>
        <div className="cpv2__stat">
          <div className="cpv2__stat-l">Contestadas</div><div className="cpv2__stat-v">{answeredPct}%</div>
          <div className="cpv2__stat-spark"><Sparkline data={trends.answeredPct} color="var(--accent-green)" /></div>
        </div>
        <div className="cpv2__stat">
          <div className="cpv2__stat-l">Duración prom</div><div className="cpv2__stat-v">{formatDurationSec(avg)}</div>
          <div className="cpv2__stat-spark"><Sparkline data={trends.avgDur} color="var(--accent-violet)" /></div>
        </div>
        <div className="cpv2__stat">
          <div className="cpv2__stat-l">Perdidas</div><div className="cpv2__stat-v">{missed}</div>
          <div className="cpv2__stat-spark"><Sparkline data={trends.missed} color="var(--accent-red)" /></div>
        </div>
      </div>
      <div className="cpv2__body">
        <aside className="cpv2__left">
          <div className="cpv2__filters">
            {([["all", "Todas"], ["in", "Entrantes"], ["out", "Salientes"], ["missed", "Perdidas"]] as const).map(([k, l]) => (
              <button key={k} className={`rec-list__chip ${dir === k ? "rec-list__chip--on" : ""}`} onClick={() => setDir(k)}>{l}</button>
            ))}
            {agents.length > 1 && (
              <select className="cpv2__agent" value={agent} onChange={(e) => setAgent(e.target.value)} aria-label="Filtrar por agente">
          ...
          {month && (
            <Calendar
              y={month.y} m={month.m} counts={countsByDay} selectedDay={selectedDay} onPick={pickDay}
              onPrev={() => setMonth((c) => (c ? (c.m === 0 ? { y: c.y - 1, m: 11 } : { y: c.y, m: c.m - 1 }) : c))}
              onNext={() => setMonth((c) => (c ? (c.m === 11 ? { y: c.y + 1, m: 0 } : { y: c.y, m: c.m + 1 }) : c))}
            />
          )}
          <div className="cpv2__daylist">
            <div className="cpv2__day-h">
              {selectedDay ? format(new Date(selectedDay + "T00:00:00"), "EEEE d 'de' MMMM", { locale: es }) : "—"} · {dayRows.length} llamada{dayRows.length === 1 ? "" : "s"}
            </div>
            {dayRows.length === 0 ? (
              ...
              return (
                <button key={c.contactId} className={`cpv2__call ${on ? "cpv2__call--on" : ""}`} onClick={() => setSelectedId(c.contactId)}>
                  <span className="cpv2__call-time">{Number.isNaN(d.getTime()) ? "—" : format(d, "HH:mm")}</span>
                  <span className="cpv2__call-dir">{miss ? <PhoneMissed size={14} /> : D === "in" ? <PhoneIncoming size={14} /> : <PhoneOutgoing size={14} />}</span>
                  <span className="cpv2__call-body">
                    <span className="cpv2__call-agent">{ag || realQueueOf(c) || "Sin agente"}</span>
                    <span className="cpv2__call-sub">{miss ? "sin atender" : `${formatDurationSec(c.duration)}${ag && realQueueOf(c) ? ` · ${realQueueOf(c)}` : ""}`}</span>
                  </span>
                  {c.hasRecording && !miss && <Disc3 size={12} className="cpv2__call-rec" />}
                  <span className={`cpv2__call-pill ${miss ? "cpv2__call-pill--miss" : ""}`}>{miss ? "Perdida" : "Contestada"}</span>
                </button>
              );
            })}
          </div>
        ...
        <main className="cpv2__right">
          {selected ? (
            ...
            <div className="cpv__msg" style={{ margin: "auto" }}>Elegí una llamada del día para reproducirla.</div>
          )}
        </main>
      </div>
    </div>
...
function Calendar({ y, m, counts, selectedDay, onPick, onPrev, onNext }: {
  y: number; m: number; counts: Record<string, number>; selectedDay: string | null;
  onPick: (ymd: string) => void; onPrev: () => void; onNext: () => void;
}) {
  ...
  return (
    <div className="cal">
      <div className="cal__head">
        <button onClick={onPrev} aria-label="Mes anterior"><ChevronLeft size={15} /></button>
        <span className="cal__month">{format(first, "MMMM yyyy", { locale: es })}</span>
        <button onClick={onNext} aria-label="Mes siguiente"><ChevronRight size={15} /></button>
      </div>
      <div className="cal__wd">{WD.map((w) => <span key={w}>{w}</span>)}</div>
      <div className="cal__grid">
        {cells.map((d, i) => {
          if (d === null) return <span key={`e${i}`} className="cal__cell cal__cell--empty" />;
          const ymd = `${y}-${String(m + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
          const c = counts[ymd] || 0;
          return (
            <button key={ymd} className={`cal__cell ${ymd === selectedDay ? "cal__cell--sel" : ""} ${c ? "cal__cell--has" : ""}`} onClick={() => onPick(ymd)}>
              <span>{d}</span>
              {c > 0 && <span className="cal__cnt">{c}</span>}
            </button>
          );
        })}
      </div>
...
function CallPlayer({ row, onActiveCall }: { row: CallRow; onActiveCall?: (c: ActiveCall | null) => void }) {
  ...
  const callSentiment = useMemo(() => {
    ...
    for (const s of transcript) {
      ...
      if (k === "POSITIVE") pos++; else if (k === "NEGATIVE") neg++; else if (k === "MIXED") mix++;
    }
  ...
  useEffect(() => {
    ...
    onActiveCall({ contactId: row.contactId, segments: rawSegments, sentiment, moments: keyMoments(transcript) });
    ...
    <div className="cpv__main">
      <div className="cpv__meta">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="cpv__meta-name" style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
            {realAgentOf(row) || realQueueOf(row) || "Llamada"}{realAgentOf(row) && realQueueOf(row) ? <span className="muted" style={{ fontWeight: 400, fontSize: 11 }}> · {realQueueOf(row)}</span> : null}
            {callSentiment && <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 99, textTransform: "capitalize", background: `color-mix(in srgb, ${callSentiment.color} 16%, transparent)`, color: callSentiment.color }}>{callSentiment.label}</span>}
          </div>
