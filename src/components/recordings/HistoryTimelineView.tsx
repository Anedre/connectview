...
function channelIcon(channel?: string) {
  ...
  if (c.includes("llam") || c.includes("call") || c.includes("voice")) return <Icon.Phone size={13} />;
  if (c.includes("correo") || c.includes("email") || c.includes("mail")) return <Icon.Mail size={13} />;
...
function buildTimeline(vox: VoxHistoryEvent[], acts: SfActivity[]): TimelineItem[] {
  ...
  vox.forEach((ev, i) => {
    ...
    if (ev.type === "interaccion") {
      ...
      title = ev.channel === "Salesforce" ? `Actualizado desde Salesforce${ev.stageLabel ? ` · ${ev.stageLabel}` : ""}` : `Etapa → ${ev.stageLabel || ev.stageId || "?"}`;
    ...
    items.push({ key: `v${i}`, ts: ev.ts || "", icon, title, detail: ev.summary || ev.notes, meta: meta || undefined, src: "vox", tone });
  });
  acts.forEach((a) => {
...
export function HistoryTimelineView({ phone, name }: { phone: string | null; name: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        if (ep?.manageLeads) {
          const r = await fetch(`${ep.manageLeads}?phone=${encodeURIComponent(phone)}`, { signal: ctrl.signal });
          const j = await r.json();
          const lead = (j.leads || [])[0];
          setVox(Array.isArray(lead?.history) ? lead.history : []);
        }
      } catch { /* sin historial Vox */ }
  ...
  if (state === "idle") {
    return (
      <div style={{ flex: 1, display: "grid", placeItems: "center", color: "var(--text-3)", fontSize: 13, padding: 24, textAlign: "center" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border-1)", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          <div className="row" style={{ gap: 6, marginTop: 3, flexWrap: "wrap" }}>
            ...
            {sf?.found && L.Status ? (
              <span className="chip" style={{ height: 18, fontSize: 9.5 }}>{String(L.Status)}</span>
            ) : null}
          </div>
        </div>
        {sf?.found && sf.lightningUrl && L.Id ? (
          <a href={`${sf.lightningUrl}/lightning/r/Lead/${String(L.Id)}/view`} target="_blank" rel="noreferrer" className="btn btn--sm" style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
      ...
      <div style={{ padding: 16 }}>
        {state === "loading" ? (
          ...
          <div className="muted" style={{ fontSize: 13 }}>Sin actividad registrada todavía para este lead.</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {timeline.map((it, idx) => (
              <div key={it.key} className="row" style={{ gap: 12, alignItems: "stretch" }}>
                {/* Rail con punto + línea */}
                <div style={{ flex: "0 0 auto", display: "flex", flexDirection: "column", alignItems: "center", width: 30 }}>
                  <span style={{ width: 28, height: 28, borderRadius: "50%", display: "grid", placeItems: "center", background: it.src === "sf" ? "rgba(0,161,224,0.14)" : "var(--bg-2)", color: it.tone, border: `1px solid ${it.tone}55` }}>
                    {it.icon}
                  </span>
                  {idx < timeline.length - 1 && <span style={{ flex: 1, width: 2, background: "var(--border-1)", marginTop: 2, minHeight: 8 }} />}
                </div>
                <div style={{ minWidth: 0, flex: 1, paddingBottom: 12 }}>
                  <div className="row" style={{ gap: 8, justifyContent: "space-between" }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-1)" }}>{it.title}</span>
                    <span className="muted" style={{ fontSize: 10.5, flex: "0 0 auto" }}>
                      {(it.ts || "").slice(0, 10)}{it.src === "sf" ? " · SF" : ""}
                    </span>
                  </div>
                  {it.detail ? <div className="muted" style={{ fontSize: 12, whiteSpace: "pre-wrap", marginTop: 3 }}>{it.detail}</div> : null}
                  {it.meta ? <div className="muted" style={{ fontSize: 10.5, marginTop: 3 }}>{it.meta}</div> : null}
                </div>
              </div>
            ))}
          </div>
