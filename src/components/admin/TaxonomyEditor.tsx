...
export function TaxonomyEditor() {
   ...
   *  embudo, así que moverlos acá cambia cómo se presenta en cada cierre. */
    ...
    <div className="col" style={{ gap: 18 }}>
      ...
      <div className="row" style={{ justifyContent: "space-between", alignItems: "flex-end", gap: 16, flexWrap: "wrap" }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: "-0.01em" }}>Taxonomía de tipificación</div>
          <div className="muted" style={{ fontSize: 12.5, marginTop: 3, maxWidth: 580, lineHeight: 1.5 }}>
            Una sola taxonomía para <strong>todos los canales</strong> (voz, WhatsApp, chat, email) y todos los
            agentes. Reemplaza las de Salesforce, Chattigo y Kommo. Los cambios se aplican al instante en cada cierre.
          </div>
        </div>
      ...
      <div className="row" style={{ gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <input
          ...
          style={{ ...inputStyle, flex: 1, minWidth: 240, fontWeight: 700, fontSize: 15, padding: "10px 12px" }}
        />
        {activeDoc?.isDefault && (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 14 }}>
        ...
        <Kpi label="Positivas" value={<span style={{ color: "var(--accent-green)" }}>{posCount}</span>} color="var(--accent-green)" />
        <Kpi label="Inicial · Neg · Cierre" value={<span><span style={{ color: "var(--accent-cyan)" }}>{iniCount}</span> · <span style={{ color: "var(--accent-red)" }}>{negCount}</span> · <span style={{ color: "var(--accent-violet)" }}>{cierreCount}</span></span>} color="var(--accent-cyan)" />
      </div>
      ...
      {loading && stages.length === 0 ? (
        <Card>
          <CardBody>
            <div className="muted" style={{ padding: 24, textAlign: "center" }}>Cargando taxonomía…</div>
          </CardBody>
        </Card>
      ) : stages.length === 0 ? (
        <Card>
          <CardBody>
            <div style={{ textAlign: "center", padding: "36px 24px" }}>
              <div style={{ display: "inline-grid", placeItems: "center", width: 48, height: 48, borderRadius: 14, background: "var(--accent-cyan-soft)", color: "var(--accent-cyan)", marginBottom: 12 }}>
                <Icon.Tag size={22} />
              </div>
              <div style={{ fontWeight: 700, fontSize: 15 }}>Esta taxonomía no tiene stages</div>
              <div className="muted" style={{ fontSize: 12.5, marginTop: 4, marginBottom: 16 }}>Agregá el primero para empezar el embudo.</div>
          ...
          {stages.map((stage, idx) => {
            ...
            return (
              <div
                key={stage._key}
                data-stage-card
                onDragOver={(e) => { e.preventDefault(); if (dragKey && dragKey !== stage._key) setOverKey(stage._key); }}
                onDragLeave={() => setOverKey((k) => (k === stage._key ? null : k))}
                onDrop={(e) => { e.preventDefault(); if (dragKey) moveStage(dragKey, stage._key); setDragKey(null); setOverKey(null); }}
                style={{
                  position: "relative",
                  borderRadius: 12,
              ...
              >
                ...
                <span style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 4, background: v.color }} />
                <div style={{ padding: "14px 16px 16px 20px" }}>
                  ...
                  <div className="row" style={{ gap: 10, alignItems: "center" }}>
                    <div
                      ...
                      style={{ cursor: "grab", display: "grid", gridTemplateColumns: "repeat(2, 3px)", gap: 3, padding: "3px 2px", flex: "0 0 auto" }}
                    >
                      {Array.from({ length: 6 }).map((_, i) => (
                        <span key={i} style={{ width: 3, height: 3, borderRadius: 99, background: "var(--text-3)" }} />
                      ))}
                    </div>
                    <span style={{ display: "grid", placeItems: "center", width: 26, height: 26, borderRadius: 8, flex: "0 0 26px", background: v.soft, color: v.color, fontWeight: 800, fontSize: 12 }}>
                      {idx + 1}
                    </span>
                    <input
                      value={stage.label}
                      ...
                      style={{ flex: 1, minWidth: 0, border: "none", background: "transparent", fontSize: 15, fontWeight: 700, color: "var(--text-1)", outline: "none", padding: "4px 0" }}
                    />
                    {/* Segmented de valoración (con color) */}
                    <div className="row" style={{ gap: 3, background: "var(--bg-2)", padding: 3, borderRadius: 9, flex: "0 0 auto" }}>
                      {VALORACIONES.map((vv) => {
                        ...
                        return (
                          <button
                            ...
                            style={{
                              ...
                              padding: "5px 11px", borderRadius: 6, transition: "all .12s",
                    ...
                    <button className="btn btn--sm btn--ghost" onClick={() => removeStage(stage._key)} title="Eliminar stage">
                      <Icon.Trash size={13} />
                    </button>
                  </div>
                  ...
                  <div className="row" style={{ gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                    <input
                      ...
                      style={{ ...inputStyle, flex: 1, minWidth: 200, fontSize: 12.5, background: "var(--bg-2)" }}
                    />
                    <input
                      ...
                      style={{ ...inputStyle, width: 150, flex: "0 0 auto", fontSize: 12, background: "var(--bg-2)" }}
                    />
                  </div>

                  {/* Sub-tipificaciones como chips */}
                  <div style={{ marginTop: 13 }}>
                    <div className="muted" style={{ fontSize: 10.5, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 8 }}>
                    ...
                    <div className="row" style={{ gap: 7, flexWrap: "wrap", alignItems: "center" }}>
                      {stage.subStages.map((sub, i) => (
                        <span
                          key={i}
                          style={{ display: "inline-flex", alignItems: "center", gap: 2, background: "var(--bg-2)", border: "1px solid var(--border-1)", borderRadius: 8, padding: "4px 5px 4px 11px" }}
                        >
                          <input
                            value={sub.label}
                            onChange={(e) => patchSub(stage._key, i, e.target.value)}
                            style={{ border: "none", background: "transparent", outline: "none", fontSize: 12.5, color: "var(--text-1)", width: `${Math.max(7, sub.label.length + 1)}ch`, minWidth: 44, maxWidth: 280 }}
                          />
                          <button
                            onClick={() => removeSub(stage._key, i)}
                            title="Quitar sub-tipificación"
                            style={{ border: "none", background: "transparent", cursor: "pointer", color: "var(--text-3)", display: "grid", placeItems: "center", padding: 3, borderRadius: 5 }}
                          >
                            <Icon.Close size={11} />
                          </button>
                        </span>
                      ...
                      <button
                        ...
                        className="row"
                        style={{ gap: 4, alignItems: "center", background: "transparent", border: "1px dashed var(--border-1)", borderRadius: 8, padding: "5px 11px", cursor: "pointer", fontSize: 12, fontWeight: 600, color: "var(--text-2)" }}
                      >
...
function WrapUpPreview({ stages }: { stages: EditStage[] }) {
  ...
  const pvSelect: React.CSSProperties = {
    width: "100%", marginTop: 5, background: "var(--bg-1)", border: "1px solid var(--border-1)",
    borderRadius: 8, padding: "9px 11px", color: "var(--text-1)", fontSize: 13.5, outline: "none",
  };
  const pvLabel: React.CSSProperties = {
    fontSize: 10.5, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".05em", color: "var(--text-3)",
  };
  return (
    <div style={{ borderRadius: 12, border: "1px solid var(--border-1)", background: "linear-gradient(135deg, var(--bg-2), var(--bg-1))", padding: "15px 18px" }}>
      <div className="row" style={{ justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 12, flexWrap: "wrap" }}>
        <div className="row" style={{ gap: 9, alignItems: "center" }}>
          <span style={{ display: "grid", placeItems: "center", width: 28, height: 28, borderRadius: 9, background: "var(--accent-violet-soft)", color: "var(--accent-violet)", flex: "0 0 auto" }}>
            <Icon.Tag size={15} />
          </span>
          <div>
            <div style={{ fontWeight: 700, fontSize: 13 }}>Vista previa del cierre</div>
            <div className="muted" style={{ fontSize: 11.5 }}>Así lo ve el agente al tipificar un contacto</div>
          </div>
        </div>
      ...
      <div className="row" style={{ gap: 12, flexWrap: "wrap", alignItems: "flex-start" }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <select value={sIdx} onChange={(e) => { setSi(Number(e.target.value)); setSsi(0); }} style={pvSelect}>
            {stages.map((s, i) => <option key={s._key} value={i}>{s.label || "(sin nombre)"}</option>)}
          </select>
        </div>
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          >
            {subs.length === 0
              ...
              : subs.map((ss, i) => <option key={i} value={i}>{ss.label || "(sin nombre)"}</option>)}
          </select>
        </div>
      </div>
    </div>
