    ...
    return (
      ...
      >
        ...
        <div style={{ marginTop: 4, color: "var(--text-3)" }}>
          Provisioná la infra con <code>node scripts/create-webhook-retry.mjs</code> y pega la
          Function URL de <code>get-webhook-deliveries</code> en{" "}
          <code>apiEndpoints.getWebhookDeliveries</code>.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
        ...
        {Object.entries(stats).map(([k, n]) => {
          ...
          return (
            <span
              key={k}
              className="chip"
              style={{ background: m.bg, color: m.fg, borderColor: "transparent" }}
            >
              {n} {m.label}
            </span>
          );
      ...
      {loading ? (
        ...
        <div className="muted" style={{ padding: 20, textAlign: "center", fontSize: 12.5 }}>
          Sin entregas todavía. Dispará una regla con acción <strong>webhook</strong> para ver el
          historial aquí.
        </div>
      ) : (
                    ...
                    <td style={{ padding: "8px", color: "var(--text-2)" }}>
                      {d.status === "delivered" ? (
                        <span style={{ color: "var(--accent-green)" }}>
                          HTTP {d.lastStatusCode} · {rel(d.deliveredAt)}
                        </span>
                      ) : d.status === "exhausted" ? (
                        <span title={d.lastError}>
                          agotó reintentos · {d.lastError?.slice(0, 40)}
                        </span>
                      ) : d.nextAttemptAt ? (
                        <span title={d.lastError}>
                          próximo {rel(d.nextAttemptAt)}
                          {d.lastError ? ` · ${d.lastError.slice(0, 30)}` : ""}
                        </span>
                      ) : (
