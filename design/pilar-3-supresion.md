...
- **`connectview-hsm-sends`**: ya tiene `phone` + `sentAt` por cada WhatsApp → con un **GSI `byPhone`** sirve para contar envíos en ventana (R6 + frecuencia). _(El mismo GSI que el Pilar 2 quería para escribir WhatsApp-out al ledger.)_
...
**Recomendación:** **(A) librería compartida** para la _enforcement_ (camino caliente, lectura) + un Lambda **`manage-suppression`** delgado solo para la _config/CRUD_ de la lista DNC y reglas (camino frío, escritura, raro) — misma separación que el resto de Configuración. _(A confirmar.)_
...
interface SendCheck {
  ...
  programId?: string;
  tenantId?: string;
  ignoreFrequency?: boolean; /* override supervisor */
}
...
async function evaluateBatch(
  ...
  summary: { willSend: number; excluded: Record<string, number> };
}>; // preview honesto
...
- **STOP/BAJA en `whatsapp-meta-webhook`**: detecta keyword en inbound → `recordOptOut(channel=whatsapp)` + auto-reply de confirmación + evento en `lead.history` (`type:"note"`); **ALTA/START → re-alta** (`removeSuppression` + confirma). Helper nuevo `getLeadByPhone` en `leadSync` para ligar el opt-out al timeline. _(Wired+desplegado; el E2E con inbound real depende de tenant en modo `meta`.)_
...
- _Entrega visible: un STOP por WhatsApp suprime futuros HSM; lista DNC en Configuración; ningún HSM sale a un opt-out._

**Fase B — Anti-doble-envío + frecuencia + preview honesto (corazón de R6) — ✅ HECHA y verificada en vivo (2026-06-19):**

- **GSI `byPhone`** en `connectview-hsm-sends` (`phoneDigits` HASH + `sentAt` RANGE, INCLUDE status) → conteo en ventana. Los 2 senders escriben `phoneDigits` (normalizado) en cada record. _(Backfill async; los HSM nuevos ya lo llevan.)_
...
- _Entrega: el wizard muestra "X excluidos" antes de enviar; el doble-envío es imposible por política._
...
- _Entrega: una sola política de contacto across canales; números malos en cuarentena sin que Adriana los cace en el Excel._
...
> **🎯 PILAR 3 · FASE C — HECHA y verificada en vivo (2026-06-30).** Cumplimiento que faltaba: gate de **voz** + **no-contactar-tras-conversión** + **SF opt-out push**.
>
> - **(a) Gate de voz en el dialer:** `campaign-dialer` ahora pasa por `evaluateSend(channel:"voice")` ANTES de cada `StartOutboundVoiceContact` (en los DOS callers: bucket por-agente y legacy single-pool), justo tras el claim atómico `markAsDialing`. Suprimido → estado TERMINAL `suppressed` (drena la campaña, no reintenta), fail-open si el motor falla. Reusa las causas duras (opt-out/DNC/cuarentena channel-scoped a "voice") + quiet-hours de voz (ya soportadas en `applyPolicy`). `markWhatsAppSuppressed`→`markSuppressed` (genérico).
> - **(b) No contactar tras conversión:** `_shared/suppression.ts` += status `converted` (4ª causa dura, `hardVerdict`→`blockedBy:"converted"`) + source `conversion` + `recordConversion(dynamo,phone,{tenantId})` (gated por la regla `suppressAfterConversion`, opt-in). Hook en `save-agent-notes`: al cerrar la gestión con `valoracion:"cierre"` → registra el teléfono como `converted` channel `all` (best-effort, nunca rompe el save). El gate (voz+WhatsApp+email) ya no lo recontacta. UI: chip "Convertido" + source "Conversión (lead ganado)" en `SuppressionManager`.
> - **(c) SF opt-out push:** `leadSync.pushDoNotCallToSalesforce(phone,doNotCall,{voxLeadId})` (match por External Id→teléfono, reusa `phoneEmailClauses`/`soql`/`updateSObject`, best-effort) cableado en `whatsapp-meta-webhook`: STOP→`DoNotCall=true`, ALTA→`false` (simétrico). El tenant BYO usa su org (`setActiveTenant`).
> - **Verificado en vivo (Browser 1, tenant real):** ① voz a un número limpio → `willSend:1`; a un DNC → `willSend:0` (gate de voz activo). ② `save-agent-notes valoracion:"cierre"` con la regla ON → entró entrada `status:"converted", source:"conversion"` y el preview de voz la excluyó como `converted:1`. Datos de prueba limpiados + regla revertida. **GOTCHA:** los 6 Lambdas que bundlean `_shared/suppression.ts` (`campaign-dialer`, `save-agent-notes`, `whatsapp-meta-webhook`, `manage-suppression`, `send-whatsapp-template`, `send-whatsapp-flow`) hay que re-desplegarlos al tocar la lib, si no el `hardVerdict` viejo etiqueta `converted` como `dnc` (bloquea igual, pero reporta mal).
> - _Pendiente menor:_ live-test del SF `DoNotCall` write (reusa el match ya verificado del Pilar 10; necesita un STOP en modo-meta + lead SF). Frecuencia de voz → v2.
...
4. _(menor)_ **Frecuencia:** GSI `byPhone` sobre `hsm-sends` (computar, recomendado) vs tabla de contadores dedicada.
...
4. _(menor, no preguntada — se construye así salvo objeción):_ frecuencia computada con **GSI `byPhone` sobre `connectview-hsm-sends`** (no tabla de contadores).
